<?php
/**
 * terada functions and definitions
 *
 ...
 */
function terada_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on terada, use a find and replace
		* to change 'terada' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'terada', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'terada' ),
		)
	);
...
add_action( 'after_setup_theme', 'terada_setup' );
 ...
 */
function terada_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'terada_content_width', 640 );
}
add_action( 'after_setup_theme', 'terada_content_width', 0 );
 ...
 */
function terada_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'terada' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'terada' ),
...
add_action( 'widgets_init', 'terada_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function terada_scripts() {
	wp_enqueue_style( 'terada-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'terada-style', 'rtl', 'replace' );

	wp_enqueue_script( 'terada-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'terada_scripts' );
