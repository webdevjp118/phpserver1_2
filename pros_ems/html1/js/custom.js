...
$(function(){
	$(window).load(function () {
		if($('#elactor1').length > 0 && $('#elactor2').length > 0) {
			setTimeout(function () {
				...
				theater
					.addActor('elactor1', { speed: 0.8, accuracy: 1 })
					.addActor('elactor2', { speed: 0.8, accuracy: 1 })
					.addScene('elactor1:FA・省力化機器装置の設計・製作')
					.addScene('elactor2:電子部品の試作、小ロット生産')
			}, 1000);
		}
	});
});
